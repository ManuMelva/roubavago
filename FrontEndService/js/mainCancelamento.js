            ...
            if (responseReserva.ok) {
              ...
              checkRooms.forEach(reserve => {
                ...
                if ((dataIni.getTime() === checkinDate.getTime()) || (dataFin.getTime() === checkOutDate.getTime() && dataIni.getTime() === checkinDate.getTime())) {
                  ...
                  <a class="book-now text-center" href="#"><i class="ti-calendar" id="cancelar"></i> Cancelar</a>
                              <input type="hidden" name="roomId" value="${room.id}">
                              <input type="hidden" name="reservaId" value="${reserve.idreserva}">
                  </div>
                  <div class="desc">
      ...
      addedNodes.forEach(newNode => {
        ...
        reservarButtons.forEach(button => {
          button.addEventListener('click', async (event) => {
            ...
            const reserveIdInput = hotelGridElement.querySelector('input[name="reservaId"]');
            const roomId = roomIdInput.value;
            const reserveId = reserveIdInput.value;


            const url = new URL('FrontEndService/cancelaInformacoes.html', window.location.origin);
            url.searchParams.append('hotelId', hotelId);
            url.searchParams.append('roomId', roomId);
            url.searchParams.append('reservaId', reserveId);

            const confirmReservation = confirm('Confirmar Cancelamento?');
